...
const getExanteAssetListFromTransactions = (
  ...
  if (tradeTransactions.length) {
    const tradeTransactionsNormalizedByTicker =
      normalizeArrayOfObjectsBy<Transaction>(
        tradeTransactions,
        "ticker"
      ) as NormalizedTransactionsByTicker

    const nonTradeTransactionsNormalizedByTicker =
      normalizeArrayOfObjectsBy<NonTradeTransaction>(
        nonTradeTransactions,
        "ticker"
      ) as NormalizedTransactionsByTicker<NonTradeTransaction>

    const openedAndClosedPositionsByTicker =
      getAllPositionsByBrokerageTransactions(
