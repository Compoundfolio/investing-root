...
const updateObjectFromArrayOfObjects = <T>(
  ...
  const index = arrayOfObjects.findIndex(existingPortfolio => existingPortfolio[updateBy] === objectToUpdate[updateBy])
  ...
  if (index === -1) {
    console.error("You are tried to update undefined object in the list or objects ...")
