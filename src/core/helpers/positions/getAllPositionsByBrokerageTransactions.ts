...
const getAllPositionsByBrokerageTransactions = (
  ...
  Object.entries(tradeTransactionsByTicker).forEach(
    ([ticker, transactionsList]: [Ticker, Transaction[]]) => {
      const sharesAmount = getSharesAmount(transactionsList)
      const openPositionPrice = getCurrentPositionPrice(
        transactionsList,
        tickersWithOpenPositionMarketPriceDictionary
      )
      ...
      const shareMarketPrice =
        tickersWithOpenPositionMarketPriceDictionary[ticker] ?? 0
      const nonTradeTransactionsList = nonTradeTransactions[ticker]

      const payedDividendTransactionsWithoutTax =
        getNonTradeTransactions<"DIVIDEND">(
          nonTradeTransactionsList,
          "DIVIDEND"
        )
      const payedDividendTaxTransactions = getNonTradeTransactions<"TAX">(
        nonTradeTransactionsList,
        "TAX"
      )
      const payedBrokerageCommissionsTransactions =
        getNonTradeTransactions<"COMMISSION">(
          nonTradeTransactionsList,
          "COMMISSION"
        )
      ...
      !isSoldOut &&
        console.log(
          averagePrice,
          shareMarketPrice,
          getPercentageDifference(shareMarketPrice, averagePrice) + "%"
        )
      ...
      const positionData: AssetPosition = {
        ...
        actualPositionPrice: isSoldOut
          ? 0
          : parseNumberToFixed2(actualPositionPrice),
