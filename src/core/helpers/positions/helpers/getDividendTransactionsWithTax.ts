...
const getDividendTransactionsWithTax = (
  ...
  const dividendTaxes = taxTransactions.filter(taxTransaction => !!taxTransaction.parentId)
  const divTaxesSumByParentId = normalizeArrayOfObjectsBy(dividendTaxes, "parentId") as { [K: ID]: NonTradeTransaction<"TAX">[] }


  const dividendTransactionsWithTax: NonTradeTransaction<"DIVIDEND">[] = divTransactions.map(divTransaction => {
    return {
      ...divTransaction,
      // TODO: Reimplement divTaxesSumByParentId[divTransaction.id][0]
      price: parseNumberToFixed2(
        // TODO: Refactor
        divTransaction.price - Math.abs(divTaxesSumByParentId?.[divTransaction?.id]?.[0]?.price ?? Number(divTransaction.comment.split('tax ').pop().split(' USD')[0]))
      )
    }
  })
