...
const ShortcutHelper = ({ keyShortcuts, onClick }: IShortcutHelper) => {
  const [activeShortcuts, setActiveShortcuts] =
    useState<KeyShortcut["eventKey"][]>(initialState)
  const shortcutTriggerEventKeyName = isMacOs()
    ? commandButton.eventKey
    : ctrlButton.eventKey
  ...
  const handleKeyUnPress = useCallback(
    (event: KeyboardEvent) => {
  ...
  return (
    ...
    >
      ...
      {keyShortcuts.map((keyShortcut, index) => (
        <div key={keyShortcut.keyName} className="flex items-center gap-2">
          <button
            className={`bg-black overflow-hidden relative h-10 px-3 rounded-lg flex justify-center align-center border border-white p-[4px] transition-all duration-75 top-0 ${
              activeShortcuts.includes(keyShortcut.eventKey) && "top-1"
            }`}
            // onClick={onClick}
          >
            {keyShortcut.keyName}
