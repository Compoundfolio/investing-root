...
interface IBrokerageMultiSelector {
  ...
  setSelectedBrokerageOptions: React.Dispatch<
...
const BrokerageMultiSelector = ({
  ...
  const selectedOptions: Option[] = useMemo(() => {
    const listOfSelectedBrokerageNames = selectedBrokerageOptions?.map(
      ({ title }) => title
    )
    const selectedOptions = allPossibleOptions?.filter(({ value }) =>
      listOfSelectedBrokerageNames?.includes(value)
    )
