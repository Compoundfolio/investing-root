...
const DivStatsBarChart = ({
  ...
  const yearDivGrowthPercentage = parseNumberToFixed2(
    (yearDivs! / pervYearDivs!) * 100
  )
  ...
  return (
    ...
    <section {...restProps} style={barChartContainerStyle({ openedInModal })}>
      ...
      <ResponsiveBar
        ...
        fill={[
          {
            match: { id: "receivedDividendAmount" },
            id: "receivedDivsGradient",
          },
        ]}
        borderColor={{
          from: "color",
        ...
        labelSkipHeight={9}
        label={({ id, value }) =>
          id === "estimatedNotReceivedDividendAmount"
            ? `~$${value}`
            : `$${value}`
        } // TODO: Currency
        motionConfig="slow"
