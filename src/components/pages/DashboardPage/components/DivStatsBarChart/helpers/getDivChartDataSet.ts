...
const getDivChartDataSet = (allPositions: AssetPosition[]): DivChartDataSet => {
  ...
  allPositions.forEach((openPosition) => {
    const divDataNormalizedByYears =
      getDivChartDataSetNormalizedByShortMonthName(
        openPosition.payedDividendTransactions
      )

    Object.entries(divDataNormalizedByYears).forEach(
      ([year, dividendMonths]) => {
        ...
        dividendMonths.forEach((dividendMonth) => {
          const index: number = divChartDataSet[year].findIndex(
            (divMonth) => divMonth.month === dividendMonth.month
          )
          divChartDataSet[year][index].receivedDividendAmount =
            parseNumberToFixed2(
              divChartDataSet[year][index].receivedDividendAmount +
                dividendMonth.receivedDividendAmount
            )
        })
      }
    )
