...
const calcDiv = (dividendList: Dividend[]): number => {
  return dividendList?.reduce(
    (prevDivPrice, currentDiv) => prevDivPrice + currentDiv.price,
    0
  )
}

const getDividendsGainsXY = (
  dividends: Dividends
): NormalizedValueChartDataSet => {
  const divsXY = Object.entries(dividends)
  ...
  const normalizedDepositsAndWithdrawalsPricesByDate =
    normalizeArrayOfObjectsBy(divsXY, "x") as NormalizedValueChartDataSet
