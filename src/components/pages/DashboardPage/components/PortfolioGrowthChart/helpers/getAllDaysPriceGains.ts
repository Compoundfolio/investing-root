...
const getFirstBuyDate = (
  ...
  allPositions.forEach(({ sharesAmount, allTradeTransactions }) => {
    allTradeTransactions
      ...
      .sort(oldDatesFirst)[OLDEST].date
  })
}
...
const getAllDaysPriceGains = async (
  ...
  const priceHistoryRequests = openPositions.map(
    ({ ticker, allTradeTransactions }) => {
      allTradeTransactions

      return MarketAPI.getTickerPricesHistoryForByDatePeriod(ticker, {
